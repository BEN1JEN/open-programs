...
+       
  ...
  local args = userInput
  local text = "Hello"
  if args then
    text = tostring(args)
  else
    print("No text specified, using default value 'Hello'.")
  end
...
function holoLineUp(x, y, z, pixel, hight)
  for i = 1, hight do
    print(i)
    hologram.set(x, y+i, z, pixel)
  end
end

function barGraph(linePcts, startx, starty, space)
  hologram.clear()

  local nextx = startx
  local nexty = starty
  local colour = 1
  for line, linePct in ipairs(linePcts) do
    local height = linePct / 100 * 32
    holoLineUp(nextx, nexty, 1, colour, height)
...
--examples:
--holoText(Hya) 
--holoLineUp(1, 1, 1, 1, 64)
--barGraph({ 50, 75, 100, 25, 125, 150, 175, 200, 300, 400 }, 1, 1, 1)